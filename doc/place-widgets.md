...
1. Hide the placeholders before the panel is drawn.

## Designing a panel

A simple idea to start designing a panel is to simply import the SVGs for the Rack widgets you're using.
Rack component library widgets are mostly composed of layers of multiple SVGs, so you'd need to make a library of design components that have the right look.

While this gives you an image that has the final look of the module,
it is a lot of inital work to create the design components, and you can't leave them in your shipped panel.
It adds a lot of size and complexity to the SVG and in some cases won't be perfectly hidden by actual widgets.
...
The element ids include the name of the Rack component they correspond to.
The wireframe file is highly simplified from what Inkscape would write, and amenable to copy and paste using a text editor.
...
Or, you can use _File_ / _Revert File_ to reload it just like in Inkscape.
...
Once you've edited an element id outside of Inkscape, Inkscape is good about preserving the id you authored.
...
In these examples we use the prefix `k:`.
You can use any prefix you like and it isn't required to use any kind of separator like the colon (`:`) we use here -- it just has to be the common prefix.
...
If you have some elements that you want to position with and also leave visible on the panel, you can use a different prefix for shown positioning elements.
For example:

```cpp
svg_query::addBounds(layout, "k:", bounds, true); // hidden positioning
svg_query::addBounds(layout, "v:", bounds, false); // visible positioning
```
...
If a widget ends up positioned where another widget should be, look for an unintentional duplication of element ids.
More than once, I've forgotten to update the id of a copy/pasted element, causing this symptom.

If we query for an individual id that isn't found, `itemBounds` returns an (Infinity,Infinity,0,0) rectangle.
...
> **#d TODO** Add examples, hot-swap svg integration
